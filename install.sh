...
token=$2
node_id=$3
server_name='gencloud'
folder="gencloud-${server_name}"
...
#install
last_version=$(curl -Ls "https://api.github.com/repos/gencl6ud/${server_name}/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
wget -N --no-check-certificate  https://github.com/gencl6ud/${server_name}/releases/download/${last_version}/${server_name}-linux-64.zip
if [[ $? -ne 0 ]]; then
